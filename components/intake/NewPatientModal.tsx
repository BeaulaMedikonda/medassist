  ...
  return (
    <Modal title="New patient intake" onClose={onClose} maxWidth="3xl">
      <div className="space-y-5">
        ...
        <Section title="2 · Vitals (optional)">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <Vital
              ...
              onChange={(v) => setVitals((s) => ({ ...s, bp_systolic: v }))}
            />
            <Vital
              label="BP dia"
              suffix="mmHg"
              value={vitals.bp_diastolic}
              onChange={(v) => setVitals((s) => ({ ...s, bp_diastolic: v }))}
            />
            <Vital
              label="Pulse"
              suffix="bpm"
            ...
            <Vital
              ...
              onChange={(v) => setVitals((s) => ({ ...s, temperature_f: v }))}
            />
            <Vital
              label="SpO₂"
              suffix="%"
            ...
            <Vital
              ...
              onChange={(v) => setVitals((s) => ({ ...s, weight_kg: v }))}
            />
          </div>
          <TextArea
            label="Chief complaint"
        ...
        >
          {doctors.length === 0 ? (
            ...
            <>
              {assignments.length > 0 ? (
                <div className="mb-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-ink-800 dark:bg-ink-900/40">
                  <label className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-ink-500">
                    Appointment date
                  </label>
                  <input
              ...
              <ul className="space-y-2">
                {assignments.map((a) => {
                  ...
                  return (
                    <li
                      key={a.doctor_id}
                      className="rounded-xl border border-brand-300 bg-brand-50/40 p-3 dark:border-brand-800 dark:bg-brand-900/20"
                    >
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-semibold text-slate-900 dark:text-ink-100">
      ...
      <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        >
          {busy ? <Spinner /> : null}
          Save & route to {assignments.length || ""} doctor
...
function Section({
  ...
  return (
    <section>
      ...
      {subtitle ? (
        <p className="mb-3 mt-0.5 text-[11px] text-slate-500 dark:text-ink-500">
          {subtitle}
        </p>
      ) : (
        <div className="mb-3" />
    ...
    <div>
      ...
      <div className="flex items-center gap-2 rounded-lg bg-slate-50 px-2 py-1.5 dark:bg-ink-900/70">
        <input
          ...
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="w-full bg-transparent text-sm font-semibold text-slate-900 placeholder:font-normal placeholder:text-slate-400 focus:outline-none dark:text-ink-100 dark:placeholder:text-ink-600"
          placeholder="—"
        />
        {suffix ? (
          <span className="text-[11px] text-slate-400 dark:text-ink-600">{suffix}</span>
