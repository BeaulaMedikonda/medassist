...
import { getClinicBillingNotice, type ClinicSubscriptionState } from "@/lib/clinic-subscription";
...
// ─── Nav definition ───────────────────────────────────────────────────────────
 ...
 const NAV: NavItem[] = [
  ...
  },
  {
    ...
    match: (p) => p === "/emr/new" || p.startsWith("/emr/new/"),
  },
  {
    href: "/emr",
    label: "Patients",
  ...
  },
  {
    ...
    match: (p) => p.startsWith("/patient-portal-requests"),
  },
];
...
export function AppShell({
  ...
  function toggleSidebar() {
    setSidebarCollapsed((v) => {
      window.localStorage.setItem("hd-sidebar-collapsed", String(!v));
      return !v;
    });
  }
  ...
  const visibleNav = NAV.filter((n) => {
    const roleAllowed = !n.roles || n.roles.includes(role);
    const featureAllowed = !n.feature || featureFlags?.[n.feature] !== false;
  ...
  useEffect(() => {
    const hrefs = [
      ...NAV.filter((item) => {
        ...
        const featureAllowed = !item.feature || featureFlags?.[item.feature] !== false;
        return roleAllowed && featureAllowed && !item.placeholder;
      }).map((item) => item.href),
      ...((role === "doctor" || role === "medical_assistant") && featureFlags?.fax !== false ? ["/fax"] : []),
      "/settings",
    ];
    const prefetch = () => {
  ...
  const sidebarBody = (
    <>
      ...
      <div className={cn(
        ...
        sidebarCollapsed ? "justify-center px-2 pb-6 pt-6" : "gap-3 px-5 pb-8 pt-7"
      )}>
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#22c7bd] to-[#0a9ea6] text-white shadow-[0_12px_28px_rgba(14,165,164,0.28)]">
          <StethoscopeIcon />
        </div>
        {!sidebarCollapsed && (
          ...
          {visibleNav.map((n) => {
            if (n.placeholder) {
              return (
                <button
                  ...
                  type="button"
                  title={n.label}
                  className={cn("nav-link w-full cursor-default text-left", sidebarCollapsed && "justify-center px-2")}
                >
            ...
            return (
              <Link
                ...
                aria-busy={pending}
                onClick={() => handleNavIntent(n.href, active)}
                className={cn(
                  "nav-link relative",
          ...
          <div className="flex flex-col gap-0.5 border-t border-white/10 pt-4">
            {showLanguageTool ? (
              <button
                ...
                className={cn("nav-link w-full text-left", languageModalOpen && "nav-link-active", sidebarCollapsed && "justify-center px-2")}
              >
                <span className={cn("flex min-w-0 items-center", sidebarCollapsed ? "justify-center" : "gap-3")}>
                  <TranslateIcon />
      ...
      <div className="border-t border-white/10 px-2 py-3">
        ...
        <button
          ...
          className={cn(
            "mb-2 flex w-full items-center gap-2 rounded-xl py-2 text-slate-400 transition hover:bg-white/10 hover:text-white",
          ...
          {userMenu ? (
            ...
            >
              ...
              <Link
                ...
                onClick={() => handleNavIntent("/settings", pathname === "/settings")}
              >
                Profile & settings
              </Link>
              {role === "admin" ? (
                <Link
                  href="/settings/team"
                  className="block rounded-lg px-3 py-2 text-sm text-slate-300 transition hover:bg-white/10 hover:text-white"
                  onClick={() => handleNavIntent("/settings/team", pathname === "/settings/team")}
                >
                  Team management
                </Link>
              ) : null}
      ...
      <main className={cn(
        ...
        <div className="mx-auto w-full max-w-[1460px] px-4 pb-24 pt-7 sm:px-8 lg:px-12">
          {billingNotice ? (
            <div
              className={cn(
                "mb-5 rounded-lg border px-4 py-3 shadow-sm",
            ...
            >
              <div className="text-sm font-black">{billingNotice.title}</div>
              <div className="mt-1 text-sm font-semibold opacity-80">{billingNotice.message}</div>
            </div>
          ) : null}
          {pendingHref ? (
