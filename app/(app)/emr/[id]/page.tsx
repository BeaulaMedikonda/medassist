...
import Link from "next/link";
import { supabaseServer } from "@/lib/supabase/server";
import type { Patient, PatientAllergy, Visit } from "@/types/db";
import { formatDate, initials } from "@/lib/utils";
import { VisitTimeline } from "@/components/emr/VisitTimeline";
import { PatientAllergiesSection } from "./PatientAllergiesSection";
...
export default async function PatientPage({
  ...
  const [{ data: visits }, { data: allergies }] = await Promise.all([
    supabase
      .from("visits")
      .select("*")
      .eq("patient_id", id)
  ...
  const p = patient as Patient;
  const v = (visits || []) as Visit[];
  const a = (allergies || []) as PatientAllergy[];
