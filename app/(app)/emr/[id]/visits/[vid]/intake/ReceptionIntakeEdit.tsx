...
export function ReceptionIntakeEdit({
  ...
  const returnTo = `/emr/${patient.id}/visits/${visit.id}/intake`;
  const encodedReturnTo = encodeURIComponent(returnTo);
  const immunizationHref = `/immunizations?patientId=${encodeURIComponent(patient.id)}&visitId=${encodeURIComponent(visit.id)}&returnTo=${encodedReturnTo}`;
  const painMapHref = `/dashboard?tool=pain-map&patientId=${encodeURIComponent(patient.id)}&visitId=${encodeURIComponent(visit.id)}&returnTo=${encodedReturnTo}`;
      ...
      <section className="card p-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Link
            ...
            className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-brand-300 hover:bg-brand-50/50 dark:border-ink-800 dark:bg-ink-900 dark:hover:border-brand-500/60"
          >
            <div className="text-sm font-bold text-slate-900 dark:text-ink-100">Immunization</div>
            <div className="mt-1 text-xs font-medium text-slate-500 dark:text-ink-500">
              Add or update vaccine records for this visit
            </div>
          </Link>
          <Link
            href={painMapHref}
            className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-brand-300 hover:bg-brand-50/50 dark:border-ink-800 dark:bg-ink-900 dark:hover:border-brand-500/60"
          >
            <div className="text-sm font-bold text-slate-900 dark:text-ink-100">Graphic Pain Map</div>
            <div className="mt-1 text-xs font-medium text-slate-500 dark:text-ink-500">
              Add or update pain markers for this visit
            </div>
          </Link>
        </div>
      </section>
