...
async function getMe(
  ...
  // New logic:
  // doctors.id = staff row id
  // doctors.auth_user_id = Supabase Auth user id
  const { data } = await sb
    .from("doctors")
    .select("*")
...
async function countActiveRowsForAuthUser(
  admin: ReturnType<typeof supabaseAdmin>,
  authUserId: string,
): Promise<number> {
...
// body: { role: "doctor" | "medical_assistant" | "admin" }
export async function PATCH(
  req: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  try {
    ...
    // Do not allow removing the last admin role.
    if (target.role === "admin" && role !== "admin") {
      const adminCount = await countAdminsInClinic(admin, me.clinic_id);
...
// - id from URL is doctors.id, not Auth user id.
// - Remove only this role/staff row from the clinic by setting clinic_id = null.
// - Do not delete Supabase Auth user if the same auth_user_id still has other active roles.
// - Delete Auth user only when no active doctors rows remain for that auth_user_id.
export async function DELETE(
  req: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  try {
    ...
    const removingSelf = target.id === me.id;
    ...
    const authUserId =
      typeof target.auth_user_id === "string" ? target.auth_user_id : null;
 
    // 1. Soft remove only this role row from this clinic.
    // This keeps old patients/visits/appointments history safe.
    const { error: rowErr } = await admin
      .from("doctors")
      .update({
        clinic_id: null,
    ...
    // 2. Delete Supabase Auth user only if no other active roles remain.
    if (authUserId) {
      const activeRowsLeft = await countActiveRowsForAuthUser(admin, authUserId);
    ...
    return NextResponse.json({
      ...
      removed: {
        id,
        self: removingSelf,
        auth_user_deleted: deletedAuthUser,
      },
    });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : "Unknown error";
